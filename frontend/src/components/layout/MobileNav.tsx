...
export function getNav(pathname: string) {
  if (pathname.startsWith('/librarian')) return LIBRARIAN_NAV
  if (pathname.startsWith('/admin')) return ADMIN_NAV
  return READER_NAV
}

const PUBLIC_READER_HREFS = ['/reader/books']
...
export default function MobileNav({ isGuest }: { isGuest?: boolean }) {
  ...
  if (isGuest && pathname.startsWith('/reader')) {
    nav = READER_NAV.filter(i => PUBLIC_READER_HREFS.includes(i.href))
  }

  // Ẩn MobileNav ở trang auth và trang public
  if (pathname.startsWith('/auth') || pathname === '/') return null

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-lg border-t border-amber-100/80 md:hidden safe-area-bottom">
