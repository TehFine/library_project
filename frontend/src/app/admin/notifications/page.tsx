...
import { Eye, Save, Clock, Rocket, Send, History, X, Search, UserCheck } from 'lucide-react'
...
export default function BulkNotificationsPage() {
  ...
  // Derived: recipients array for display
  ...
  const handleSaveDraft = async () => {
    if (!subject.trim() || !content.trim()) {
      toast.error('Vui lòng nhập tiêu đề và nội dung trước khi lưu.')
      return
    }
    try {
      await notificationApi.create({
  ...
  const handleSendTest = async () => {
    // Create a draft first, then send test
    setSendingTest(true)
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        />
        <div className="flex gap-1 bg-slate-100 rounded-2xl p-1 shrink-0 self-start sm:self-auto overflow-x-auto scrollbar-hide max-w-full pt-1">
          <button
          ...
          >
            <Rocket className="w-3 h-3 sm:w-3.5 sm:h-3.5 inline mr-1" /> Soạn thảo
          </button>
          <button
          ...
          >
            <History className="w-3 h-3 sm:w-3.5 sm:h-3.5 inline mr-1" /> Lịch sử gửi
          </button>
        </div>
      </div>
        ...
        <Card padding="lg" className="space-y-8">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
              <span className="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xs">1</span>
              Đối tượng nhận tin
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 px-2 sm:px-8">
              {loadingCounts ? (
                ...
                targetOptions.map(opt => (
                  <label key={opt.id} className={`flex items-center justify-between p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 transition-all cursor-pointer ${
                    ...
                    <span className={`text-[9px] sm:text-[10px] font-black tracking-widest uppercase ${opt.color} whitespace-nowrap`}>
              ...
              {recipients.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {recipients.map(r => (
                    <span
                      key={r}
                      className="inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-700 text-[10px] font-bold rounded-lg border border-amber-100"
                    >
                      {r}
                      <button
                        type="button"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="space-y-4">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <span className="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xs">2</span>
                  Kênh gửi & Mẫu tin
                </h3>
             ...
             <div className="space-y-4">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  <span className="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xs">3</span>
                  Tiêu đề thông báo
                </h3>
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
              <span className="w-6 h-6 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center text-xs">4</span>
              Nội dung thông báo
            </h3>
            <div className="px-2 sm:px-8 space-y-3">
               <div className="relative group">
                  <textarea
                    className="w-full h-48 rounded-2xl bg-slate-50 border border-slate-200 p-6 text-sm text-slate-700 outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-sans leading-relaxed resize-none"
               ...
               <div className="flex flex-wrap gap-1.5 sm:gap-2 items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-1 sm:mr-2 w-full sm:w-auto">Biến động:</span>
          ...
          <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
             ...
             <Button
               variant="primary"
               className="px-12 font-bold shadow-glow shadow-amber-500/30"
               onClick={handleSend}
               disabled={sending}
             >
                {sending ? <><Clock className="w-4 h-4 animate-spin" /> Đang gửi...</> : <><Rocket className="w-4 h-4" /> Gửi ngay bây giờ</>}
             </Button>
          </div>
        </Card>
      ) : (
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-4">Số người nhận</th>
            ...
            <tbody className="divide-y divide-slate-50 text-sm">
              {loadingHistory ? (
                ...
                <tr><td colSpan={5} className="text-center py-10 text-slate-400">Chưa có thông báo nào được gửi</td></tr>
              ) : (
                history.map(n => (
         ...
         <div className="space-y-6">
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
               <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                 Gửi đến: <span className="text-slate-900 ml-2">
                   {targetCounts ? `${targetCounts[target].toLocaleString('vi-VN')} người` : '...'}
                 </span>
               </p>
            ...
            <div className="p-8 rounded-2xl bg-white border border-slate-100 shadow-inner min-h-[300px] text-sm text-slate-700 leading-relaxed font-sans whitespace-pre-line">
               <div className="flex justify-center mb-8">
                  <div className="w-12 h-12 bg-amber-600 rounded-xl flex items-center justify-center text-white text-xl font-bold shadow-lg">B</div>
               </div>
               {previewContent}
            </div>
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
               ...
               <Button variant="primary" onClick={handleSendTest} disabled={sendingTest}>
                  {sendingTest ? <><Clock className="w-4 h-4 animate-spin" /> Đang gửi...</> : <><Rocket className="w-4 h-4" /> Gửi thử bản này</>}
               </Button>
            </div>
         </div>
      </Modal>
